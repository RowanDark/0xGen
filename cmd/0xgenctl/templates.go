...
func runTemplatesSave(args []string) int {
	...
	enableAI := fs.Bool("ai", false, "enable pattern-based features")
	aiPayloads := fs.Bool("ai-payloads", false, "enable context-aware payloads")
	aiClassify := fs.Bool("ai-classify", false, "enable pattern-based classification")
	aiFindings := fs.Bool("ai-findings", false, "enable findings correlation")
